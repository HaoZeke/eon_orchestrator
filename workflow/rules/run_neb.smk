...
rule run_neb:
    ...
    """
    input:
        reactant=config['paths']['endpoints'] + '/{system}/reactant.con',
        product=config['paths']['endpoints'] + '/{system}/product.con',
        model=f"{config['paths']['models']}/{config['model']['name']}.pt",
    output:
        results_dat=config['paths']['neb'] + '/{system}/results.dat',
        neb_con=config['paths']['neb'] + '/{system}/neb.con',
        neb_dat=config['paths']['neb'] + '/{system}/neb.dat',
    params:
        device=config['compute']['device'],
        opath=config['paths']['neb'] + "/{system}",
    ...
    run:
        ...
        neb_settings = {
            ...
            "Nudged Elastic Band": {
                ...
                # Energy-weighted springs for better barrier resolution
                ...
                # Climbing image with off-path refinement
                "climbing_image_method": str(params.climbing_image_method).lower(),
                "climbing_image_converged_only": "true",
                "ci_after": 0.5,  # Absolute convergence before CI activates
                "ci_after_rel": params.ci_after_rel,
                # MMF (dimer) refinement at saddle
                "ci_mmf": str(params.ci_mmf).lower(),
                "ci_mmf_after": 0.1,  # Absolute convergence threshold
